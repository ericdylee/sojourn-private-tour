...
**`caption.txt`에는 `output/publish_caption.md`의 릴스 절(사진 완전 표기)이 반드시 들어간다.** 화면의
`.photo-credit`은 축약형이라 라이선스 링크와 변형 내역이 빠져 있고, 그걸 뺀 조건이 캡션 동봉이었다.
캡션 없이 영상만 올리면 CC BY 4.0 표기가 미완성이다 — 화면에 크레딧이 보인다고 끝난 것이 아니다.
