...
## 현재 방향 (2026-08-05) — SIMPLE

**사진이 카드다. 타이포는 한 가지만 말하고 비킨다.** `.lay-simple` 아키타입을 쓴다.

```
사진 풀블리드(원색 유지) · 검정 스크림(상·하)
헤드라인 Montserrat 800 80px 2줄 (핵심 구절 1개만 --orange)
하단 지지문 1줄 Montserrat 600 32px
우상단 배지 --orange 바탕 + ink 글자 · 우측 세로 핸들
```

**서체:** 헤드라인·지지문은 `--voice` = **Montserrat**. 영어권 마케팅에서 가장 널리 쓰이고, Poppins보다 x-height가 커서 사진 위 큰 글자 가독성이 낫다. 락업·배지·워터마크·크레딧 같은 **고정 요소는 Inter 유지** — 20~24px에서는 Inter가 낫다.

**액센트:** `--orange #FF7A29`. `.lay-simple` 안에서 **coral을 대체**한다(추가가 아니다 — 카드당 액센트는 여전히 1종).

| 왜 코랄이 아니라 이 오렌지인가 | 수치 |
|---|---|
| 액센트가 **글자**로 들어간다. 검정 스크림 대비 | coral 3.07:1 → orange **4.03:1** |
| 감천 사진은 파란 지붕이 지배 → 오렌지가 보색 | 사진에 묻히지 않는다 |
| 배지는 **오렌지 바탕 + ink 글자** | 흰 글자 2.6:1 vs ink **8.1:1** |

**카드당 강조는 한 구절뿐이다.** 두 개를 칠하면 첫 번째의 값이 반으로 준다.

**오렌지는 흰 글자보다 훨씬 두꺼운 스크림을 요구한다.** 흰색은 휘도 1.0이라 얇은 스크림에서도 살지만 오렌지는 0.35로 중간이라, 볕 든 벽 위에서는 그대로 사라진다(실측: 카드 02 둘째 줄 1.18:1). 그래서 상단 램프가 22%가 아니라 **35%까지 최대치 근처를 유지**한다. 그 아래로는 사진을 건드리지 않는다.

이 방향은 아래 **"디자인 원칙" 1번과 "AI 슬롭" 표의 소프트 섀도 항목을 의도적으로 유예한다.** 유예 사유는 `brand.css`의 SIMPLE 블록 주석에 있다. 요약:

| 유예한 규칙 | 이 방향에서 왜 |
|---|---|
| 초대형 워드가 레이아웃이다 (Anton 210px) | 여기서는 **사진이** 레이아웃이다. 헤드라인이 사진을 덮으면 이 방향의 존재 이유가 사라진다 |
| 브랜드 블루 지배 | 스크림이 **검정**이다. 블루 스크림은 장소의 색을 브랜드 색으로 덮는다 |
| 소프트 섀도 금지 | 사진 위 흰 글자를 떼어내는 **구조적 역할**이다. 장식 글로우가 아니고, `.t-plate` 스트로크는 이 레지스터에 너무 시끄럽다 |

**유예는 `.lay-simple` 안에서만이다.** Anton 카드에 소프트 섀도를 가져가면 그건 규칙 위반이다.

**아래 절들은 이전 방향(v1~v3)을 서술한다.** 아키타입·콜라주·두들·웨이브는 여전히 코드에 살아 있고 다른 세트에서 쓸 수 있지만, 현재 세트의 모습은 위 SIMPLE이다.
